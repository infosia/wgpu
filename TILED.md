...
- [x] Phase 11 — wgpu-core bridge to expose Phase 9 HAL through the public API
  ...
  - [~] Phase 11j — Vulkan validation cleanup for `SubpassRenderPass`. Major progress:
    ...
    - [x] Acquire-semaphore cascade. With the swapchain layout correct, the acquire semaphore is no longer signaled at the wrong time (`vkAcquireNextImageKHR-semaphore-01286` was a cascade of the swapchain issue).
    - [ ] Framebuffer-cache lifecycle (`vkDestroyImageView-imageView-01026`). `wgpu-hal/src/vulkan` caches `VkFramebuffer`s in the command encoder keyed by `attachment_identities`. When a `TextureView` is destroyed, `destroy_texture_view` removes the view but leaves the framebuffer in the cache referencing the now-dangling handle. This issue exists in upstream's `RenderPass` path too (same shared cache at `wgpu-hal/src/vulkan/command.rs:94`); it surfaces here because our example destroys views per-frame more aggressively. The example still renders correctly; the validator's complaint is spec-correct but benign in practice. A clean fix would either (1) track framebuffers per source view so destroying a view invalidates dependent FBs, or (2) drop the cache entirely (one FB create per pass, slow but simple). Left for a separate sub-phase.
