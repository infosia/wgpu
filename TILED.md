...
- [x] Phase 11 — wgpu-core bridge to expose Phase 9 HAL through the public API
  ...
  - [ ] Phase 11j — Vulkan validation cleanup for `SubpassRenderPass`. Three known remaining gaps when running with the validation layers enabled (debug build): (a) `InvalidImageLayout` -- wgpu-core's resource tracker expects attachments to be transitioned to `COLOR_ATTACHMENT_OPTIMAL` *before* the subpass pass begins, but `begin_subpass_render_pass` doesn't emit the pre-pass barriers the upstream `RenderPass` path emits. (b) `VkPresentInfoKHR-pImageIndices-01430` -- swapchain attachment's `finalLayout` should be `PRESENT_SRC_KHR` when the user passes a surface texture view, not `COLOR_ATTACHMENT_OPTIMAL`. (c) `vkDestroyImageView-imageView-01026` -- the framebuffer cache in `wgpu-hal/src/vulkan/mod.rs` keeps framebuffers referencing image views that have been destroyed on resize. The example still renders correctly at 60 FPS in release; these are validation-spec violations that don't affect output but should be cleaned up.
